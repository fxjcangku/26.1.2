...
public class ClientHandshakePacketListenerImpl implements ClientLoginPacketListener {
	...
	private void switchState(final ClientHandshakePacketListenerImpl.State toState) {
		ClientHandshakePacketListenerImpl.State newState = (ClientHandshakePacketListenerImpl.State)this.state.updateAndGet(lastState -> {
	...
	public void fillListenerSpecificCrashDetails(final CrashReport report, final CrashReportCategory connectionDetails) {
		connectionDetails.setDetail("Server type", (CrashReportDetail<String>)(() -> this.serverData != null ? this.serverData.type().toString() : "<unknown>"));
		connectionDetails.setDetail("Login phase", (CrashReportDetail<String>)(() -> ((ClientHandshakePacketListenerImpl.State)this.state.get()).toString()));
