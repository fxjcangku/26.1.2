...
public class ObjectiveCriteria {
	...
	public static final Codec<ObjectiveCriteria> CODEC = Codec.STRING
		.comapFlatMap(
			name -> (DataResult)byName(name).map(DataResult::success).orElse(DataResult.error(() -> "No scoreboard criteria with name: " + name)),
		...
		if (value != null) {
			...
			return colonPos < 0
				...
				: BuiltInRegistries.STAT_TYPE
					...
					.flatMap(statType -> getStat(statType, Identifier.bySeparator(name.substring(colonPos + 1), '.')));
		}
	}
	...
	public static enum RenderType implements StringRepresentable {
		...
		public static ObjectiveCriteria.RenderType byId(final String key) {
			return (ObjectiveCriteria.RenderType)CODEC.byName(key, INTEGER);
		}
	}
}
