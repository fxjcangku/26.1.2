...
public class NoiseRouterData {
	...
	private static void registerTerrainNoises(
		...
		DensityFunction offset = registerAndWrap(
			...
			offsetName,
			splineWithBlending(
				DensityFunctions.add(DensityFunctions.constant(-0.50375F), DensityFunctions.spline(TerrainProvider.overworldOffset(continents, erosion, ridges, amplified))),
		...
		DensityFunction unscaledJaggedness = registerAndWrap(
			context,
			jaggednessName,
			splineWithBlending(DensityFunctions.spline(TerrainProvider.overworldJaggedness(continents, erosion, weirdness, ridges, amplified)), BLENDING_JAGGEDNESS)
		);
	...
	private static DensityFunction preliminarySurfaceLevel(final DensityFunction offset, final DensityFunction factor, final boolean amplified) {
		...
		DensityFunction upperBound = remap(
			DensityFunctions.add(
				DensityFunctions.mul(DensityFunctions.constant(0.2734375), cachedFactor.invert()), DensityFunctions.mul(DensityFunctions.constant(-1.0), cachedOffset)
			),
			1.5,
			-1.5,
			-64.0,
		...
		DensityFunction density = DensityFunctions.add(
			slideOverworld(
				amplified, DensityFunctions.add(noiseGradientDensity(cachedFactor, offsetToDepth(cachedOffset)), DensityFunctions.constant(-0.703125)).clamp(-64.0, 64.0)
			),
