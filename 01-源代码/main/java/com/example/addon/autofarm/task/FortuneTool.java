...
public final class FortuneTool {
    ...
    public boolean ensure() {
        ...
        FindItemResult best = findBestFortune();
        if (best.found()) {
            prepTicks++;
            if (!preparing || prepTicks > 8) {
                if (best.isOffhand()) {
                    InvUtils.move().fromOffhand().to(mc.player.getInventory().getSelectedSlot());
                } else {
                    InvUtils.move().from(best.slot()).to(mc.player.getInventory().getSelectedSlot());
                }
                preparing = true;
                prepTicks = 0;
            }
    ...
    /** 全背包（含副手）里时运等级最高的工具，没有返回 not-found */
    private FindItemResult findBestFortune() {
        Minecraft mc = Minecraft.getInstance();
        ...
        return new FindItemResult(bestSlot, bestLevel > 0 ? 1 : 0);
    }
