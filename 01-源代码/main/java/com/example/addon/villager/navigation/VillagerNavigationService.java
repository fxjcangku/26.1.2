...
public final class VillagerNavigationService {
    ...
    public boolean pathToContainer(BlockPos container) {
        ...
        return startPath(new GoalBlock(standingPos));
    }
     ...
     * 正前方不可站立（被墙围死等极端情况）时回退 {@link GoalNear} 让 Baritone 自行找可达位置，
     * 避免精确站位不可达导致反复重新寻路、满屏路径线。</p>
     *
    ...
    public boolean pathToVillager(BlockPos villagerPos, BlockPos workstationPos) {
        if (workstationPos != null) {
            ...
            if (front != null) {
                ...
                return startPath(new GoalBlock(front));
            }
        }
        villagerStandTarget = null;
        return startPath(new GoalNear(villagerPos, 2));
    }
    ...
    /**
     * 统一发起 Baritone 寻路并重置卡死检测。
     */
    private boolean startPath(Goal goal) {
