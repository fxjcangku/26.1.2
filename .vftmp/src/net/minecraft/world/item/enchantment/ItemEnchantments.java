...
public class ItemEnchantments implements TooltipProvider {
   ...
   @Override
   public void addToTooltip(final TooltipContext context, final Consumer<Component> consumer, final TooltipFlag flag, final DataComponentGetter components) {
   ...
   private static <T> HolderSet<T> getTagOrEmpty(@Nullable final Provider registries, final ResourceKey<Registry<T>> registry, final TagKey<T> tag) {
      if (registries != null) {
         Optional<Named<T>> maybeOrder = registries.lookupOrThrow(registry).get(tag);
