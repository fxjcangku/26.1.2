...
public class SpriteLoader {
	...
	private SpriteLoader.Preparations stitch(final List<SpriteContents> sprites, final int maxMipmapLevels, final Executor executor) {
		...
		try (Zone ignored = Profiler.get().zone((Supplier<String>)(() -> "stitch " + this.location))) {
			...
			try {
				...
				category.setDetail(
					"Sprites",
					var21.getAllSprites().stream().map(s -> String.format(Locale.ROOT, "%s[%dx%d]", s.name(), s.width(), s.height())).collect(Collectors.joining(","))
				);
	...
	public CompletableFuture<SpriteLoader.Preparations> loadAndStitch(
		...
		return CompletableFuture.supplyAsync(() -> SpriteSourceList.load(manager, atlasInfoLocation).list(manager), taskExecutor)
			.thenCompose(sprites -> runSpriteSuppliers(spriteResourceLoader, sprites, taskExecutor))
			.thenApply(resources -> this.stitch(resources, maxMipmapLevels, taskExecutor));
	}
	...
	public record Preparations(
		...
		@Nullable
		public TextureAtlasSprite getSprite(final Identifier id) {
