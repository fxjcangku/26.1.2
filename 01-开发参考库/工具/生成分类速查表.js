 ...
 *
 * 用法：node 03-映射表/工具/生成分类速查表.js
 *
 * 输入：03-映射表/类名索引-26.1.2.txt
 * 输出：03-映射表/分类速查/*.txt   按功能域拆分，每个文件带中文用途注释
 *
...
if (!fs.existsSync(索引文件)) {
    console.error(`找不到类名索引：${索引文件}\n请先运行 node 03-映射表/工具/从MinecraftJAR生成索引.js。`);
    process.exit(1);
}
...
for (const 规则 of 分类规则) {
    ...
    const 头部 =
        ...
        `# 来源：03-映射表/类名索引-26.1.2.txt\n` +
        `# 生成：node 03-映射表/工具/生成分类速查表.js\n` +
        `#\n` +
