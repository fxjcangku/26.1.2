...
public final class BatchHarvestTask implements FarmTask {
    ...
    /** 已成功收割的目标清单，供 Controller 统一补种与拾取 */
    public List<FarmTarget> harvested() {
        return harvested;
    }

    /** 剩余待处理目标数（含已破坏但尚未验证的目标），供进度播报 */
    public int remaining() {
        return remaining.size() + pending.size();
    }
    ...
    public TaskResult tick() {
        ...
        for (FarmTarget t : inReach) {
            FarmPacketOps.breakBlock(t.pos(), Direction.UP);
            remaining.remove(t);
            pending.add(t);
        }
        acted = true;
