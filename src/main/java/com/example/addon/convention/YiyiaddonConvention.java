...
// ════════════════════════════════════════════════════════════════════════════
//
// 5.0 模块架构标准（2026-09-08 新增企业级架构要求）
//
// 所有自动化模块必须遵循分层架构，禁止硬编码识别逻辑：
//
// 必需层次：
//   - recognition/  识别层（至少1个识别器接口 + 1个实现 + 识别结果record）
//   - adapter/      适配器层（1个适配器接口 + 1个默认实现）
//   - task/         任务层（继承统一任务接口，调用适配器接口）
//
// 可选层次：
//   - controller/   控制器层（复杂模块使用，如原版自动农场）
//   - service/      服务层（需要独立服务时，如浇水规划）
//   - persistence/  持久化层（需要JSON配置时，如星露谷）
//   - integration/  集成层（与现有体系对接，如ID系统）
//
// 核心设计原则（强制遵守）：
//   1. 接口优先 - 所有识别逻辑必须定义接口，不得直接硬编码
//   2. 适配器隔离 - 服务器差异通过适配器隔离，不在业务代码中判断服务器类型
//   3. 依赖注入 - 模块类通过构造函数注入适配器，不直接new识别器
//
// 参考示例（已完成企业级架构升级）：
//   ✓ 星露谷农场模块 - 完整7层分离（标杆级架构）
//   ✓ 原版自动农场模块 - 新增recognition + adapter层（2026-09-08升级）
//   ✓ 挖矿模块 - 新增recognition + adapter层（2026-09-08升级）
//   ✓ 村民交易模块 - 新增recognition + adapter层（2026-09-08升级）
//   ✓ 自动箱子模块 - 新增adapter层（2026-09-08升级）
//
// 强制检查清单（新模块开发前必读）：
//   ☑ 创建recognition/目录，定义至少1个识别器接口
//   ☑ 创建adapter/目录，定义适配器接口和默认实现
//   ☑ 模块类通过构造函数注入适配器
//   ☑ 任务层调用适配器接口，不直接调用识别器
//   ☑ 所有硬编码判定逻辑移入识别器实现
//   ☑ 编译通过 + 审计通过 + 中文注释
//
// 反面示例（禁止）：
//   ✗ 在模块类中直接判断 block == Blocks.WHEAT（硬编码）
//   ✗ 在任务类中直接判断 state.getValue(AGE) == 7（硬编码）
//   ✗ 业务代码中用 if(serverType==VANILLA) 判断服务器类型（不隔离）
//
//
// 【5.1 核心功能】
...
//
// 【5.11 报告文档规范（2026-09-03 新增，全项目统一）】
//
