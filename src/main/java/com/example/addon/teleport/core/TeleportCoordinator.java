package com.example.addon.teleport.core;

import com.example.addon.teleport.geo.SurfaceScanner;
import com.example.addon.teleport.geo.WallRayScanner;
...
public final class TeleportCoordinator {
    ...
    public void onServerPacket(Packet<?> packet, LocalPlayer player) {
        if (active == null || state != TeleportState.VERIFY || packet == null || player == null) return;

        int verdict = RubberbandVerifier.classify(packet, player, active);
