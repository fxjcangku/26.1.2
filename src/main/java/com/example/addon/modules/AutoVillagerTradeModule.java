...
public class AutoVillagerTradeModule extends YiyiaddonModule {
    ...
    private final Setting<Boolean> drainMode = sgGeneral.add(new BoolSetting.Builder()
        .name("榨干模式(按钮)")
        .description("三个运行模式共用的开关，默认关闭。开启后无视各职业购买量，把你选择的物品买到目标交易全部售罄/锁死为止")
        .defaultValue(false)
        .build()
    );
    ...
    private void initializeItemSettings() {
        ...
        for (ProfessionChoice profChoice : ProfessionChoice.values()) {
            ...
            // 1. 物品选择器
            ItemListSetting itemSetting = sgGeneral.add(new ItemListSetting.Builder()
                .name(profName + "交易")
            ...
            if (profChoice == ProfessionChoice.图书管理员) {
                librarianEnchantments = sgGeneral.add(new EnchantmentListSetting.Builder()
                    .name("图书管理员附魔书")
                    .description("只选择村民能够刷出的附魔类型，交易时忽略附魔等级")
                    .defaultValue()
                    .visible(() -> profession.get() == ProfessionChoice.图书管理员)
                    .build()
                );
            }

            // 3. 购买总量（组）：加减按钮 + 可点框手输；榨干开关开启后隐藏（榨干优先，两者互斥）
            Setting<Integer> supplySetting = sgGeneral.add(new IntSetting.Builder()
                .name(profName + " 购买量(组)")
                .description("本次运行该职业物品的购买总量（组，1组=64个），买够自动收工（榨干模式下无效并自动隐藏）")
                .defaultValue(1)
                .min(1)
                .max(64)
                .noSlider()
                .visible(() -> {
                    ProfessionChoice current = profession.get();
                    return !drainMode.get() && current != null && current.name().equals(profName);
                })
                .build()
            );
            ...
            // 4. 价格上限：加减按钮 + 可点框手输
            Setting<Integer> priceSetting = sgGeneral.add(new IntSetting.Builder()
                .name(profName + " 价格上限")
    ...
    private List<String> selfCheck() {
        ...
        if (mode.get() == Mode.PIPELINE) {
            if (pipelineTasks == null || pipelineTasks.isEmpty()) {
                missing.add("§e多任务§f·未配置任何任务（至少给一个职业选择物品）");
            }
    ...
    /**
     * 构建多任务队列：遍历 13 个职业，凡「已选择物品」的职业都生成一个任务。
    ...
    private String[] buildHelpContent() {
        return HelpScreen.buildHelpContent(
            ...
            new HelpScreen.HelpSection("榨干开关 §7(按钮·默认关闭)",
