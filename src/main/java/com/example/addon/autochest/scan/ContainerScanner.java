 ...
 *
 * <p>绝对禁止每 tick 全量扫整个范围——一个半径 32 的立方体就是 27 万格。
 * 这里采用「扫描周期 + 分帧 + 稳定快照」三层缓存：
...
public final class ContainerScanner {
    ...
    /** 中心移动超过该平方距离才重启扫描（避免玩家原地小抖动反复重启） */
     ...
     *
     * <p>由模块按「扫描周期」调用，不在每 tick 反复触发。</p>
     */
    ...
    public boolean tick(BlockPos center) {
        ...
        while (budget-- > 0) {
            ...
            if (typeId != null) {
                buffer.add(new ChestTarget(pos, WorldIdentity.dimension(mc),
                    WorldIdentity.server(mc), typeId));
            }
     ...
     * ChestBlockEntity，但可按 Block 区分。</p>
     */
