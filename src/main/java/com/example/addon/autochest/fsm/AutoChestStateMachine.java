package com.example.addon.autochest.fsm;
 ...
 * </ul>
 *
 * <p>第七阶段核心机制全部保留：目标锁（同一容器同时只有一个处理任务）、多人保护
 * （其他玩家在用不抢箱）、有限重试（开箱/寻路/交互/关闭失败上限）、临时冷却
 * （连续失败/多人保护后暂时跳过）。播报遵循规范 5.9：工作状态播进度/结果，
 * 寻路过渡不播，带 lastNotifiedState 去重。</p>
 */
public final class AutoChestStateMachine {
    ...
    public AutoChestStateMachine(AutoChestModule module) {
        this.module = module;
    ...
    public void tick() {
        ...
        ScanMode mode = module.moduleSettings.scanMode.get();
    ...
    private boolean checkFatalConditions() {
        ...
        if (lastDimension != null && !lastDimension.isEmpty() && !lastDimension.equals(dim)) {
            module.pathingService.stop();
            module.interactionService.close();
            reset();
        }
        lastDimension = dim;
        return false;
    ...
    private boolean validLocked() {
        ...
        if (!lockedTarget.inCurrentDimension() || !isContainerAt(lockedTarget.pos())) {
            module.recordManager.invalidate(lockedTarget.pos(), lockedTarget.dimension());
            lockedTarget = null;
        ...
        if (module.moduleSettings.multiplayerProtect.get() && hasNearbyPlayer(lockedTarget.pos())) {
    ...
    private ChestTarget selectTarget(ScanMode mode) {
        ...
        if (mode == ScanMode.MARKER) {
            // 标点模式：只读标点，不处理普通扫描箱子
            return module.selector.select(filterAvailable(module.pointManager.pointsInCurrentDimension()),
                playerPos, dim, module.recordManager, expireMs);
        }

        List<ChestTarget> candidates = filterAvailable(module.scanner.results());
        if (mode == ScanMode.PLAYER_CONTROL) {
            // 玩家控制：只处理触发距离内的容器，玩家自己走，模块不寻路
            int trigger = module.moduleSettings.triggerDistance.get();
            int triggerSqr = trigger * trigger;
            return candidates.stream()
                .filter(c -> c.distSqr(playerPos) <= triggerSqr)
                .filter(c -> !module.recordManager.isProcessed(c.pos(), dim, c.containerType(), expireMs))
    ...
    private void advance(ScanMode mode) {
        switch (state) {
            ...
            case CALCULATE_STAND_POSITION -> {
                ...
                // 清理旧路径后计算安全站位并发起寻路
                module.pathingService.stop();
                if (!module.pathingService.pathTo(lockedTarget.pos())) {
            ...
            case PATHING -> {
                ...
                } else if (!module.pathingService.isPathing()) {
            ...
            case OPENING -> {
                ...
                } else if (module.interactionService.open(lockedTarget.pos())) {
            ...
            case READING_SLOTS -> {
                ...
                if (module.interactionService.isSynced()) {
            ...
            case TAKING -> {
                ...
                ChestInteractionService.Result result =
                    module.interactionService.withdrawOnce(module.moduleSettings.withdrawMode.get());
                switch (result) {
                    case WITHDREW -> {
                        // 继续取物
                    }
            ...
            case COMPLETED -> {
                ...
                module.recordManager.markProcessed(
    ...
    private void fail(String reason, State retryState) {
        ...
        module.notifyStatus("§e⚠ " + reason + " §8▸ 重试 " + retryCount + "/" + max);
    ...
    private void skipWithCooldown(String reason) {
        ...
        retryCount = 0;
        cooldownWait = NEXT_COOLDOWN;
        module.notifyStatus("§e⚠ " + reason + " §8▸ 暂时跳过");
        transitionTo(State.COOLDOWN);
    }
    ...
    private boolean isProcessed(ChestTarget target) {
        long expireMs = module.moduleSettings.recordExpireMinutes.get() * 60_000L;
        return module.recordManager.isProcessed(
    ...
    private boolean hasNearbyPlayer(BlockPos pos) {
        if (mc.level == null) return false;
        double range = module.moduleSettings.playerDetectDistance.get();
    ...
    private int maxRetries() {
        return Math.max(1, module.moduleSettings.maxRetries.get());
    }

    private long cooldownMs() {
        return module.moduleSettings.cooldownTicks.get() * 50L;
    }
    ...
    private void broadcast(State s) {
        ...
        String coords = YiyiaddonModule.formatCoords(
            lockedTarget.pos().getX(), lockedTarget.pos().getY(), lockedTarget.pos().getZ());
        switch (s) {
            case OPENING -> module.notifyStatus("§7正在打开容器 §8▸ " + coords);
            case TAKING -> module.notifyStatus("§7正在取物 §8▸ " + coords);
            case COMPLETED -> module.notifyStatus("§a✓ 处理完成 §8▸ " + coords);
            default -> {
                // 其余状态不播
            }
        }
